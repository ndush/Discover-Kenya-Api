class AttractionsController < ApplicationController
  ...
  radius = params[:radius] || 5000  # Default to 5000 meters if no radius is provided

  if latitude.present? && longitude.present?
    service = HereService.new(ENV['HERE_API_KEY'])

    # Fetch attractions based on latitude, longitude, and radius
    ...
    # Return all attractions if latitude and longitude are not provided
  ...
  # Set location as a Point from latitude and longitude
  ...
  @attraction.status = :pending  # Default status is pending
  ...
  # PATCH /attractions/:id/approve
def approve
  if current_user.moderator?  # Check if the current user has moderator privileges
 ...
 def reject
  if current_user.moderator?  # Check if the current user has moderator privileges
    Rails.logger.debug("Rejecting attraction with ID: #{@attraction.id}") # Add debug log
  ...
  # Set attraction for moderation actions
  def set_attraction
  ...
  # Strong parameters for creating an attraction
